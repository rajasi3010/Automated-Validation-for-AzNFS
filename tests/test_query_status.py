...
def _db(tmp_path) -> str:
    ...
    rows = [
        ...
        (("Debian", "debian-11", "11", "eastus", "x86_64"),
         "12.0.1", "Debian 12", "known_unsupported", "prod repo is missing"),
...
def test_buckets_match_the_monthly_digest_rollup(tmp_path):
    ...
    assert [d["distro_label"] for d in unsupported] == ["Debian 12"]  # CentOS excluded
...
def test_include_excluded_restores_filtered_distros(tmp_path):
    ...
    assert [d["distro_label"] for d in buckets["known_unsupported"]] == ["CentOS 7", "Debian 12"]
