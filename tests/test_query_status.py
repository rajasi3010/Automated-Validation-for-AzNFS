...
def _db(tmp_path) -> str:
    ...
    rows = [
        ...
        (("SUSE", "sles-15-sp5", "gen2", "eastus", "x86_64"),
         "2026.08", "SLES 15", "known_unsupported", "prod repo is missing"),
...
def test_buckets_match_the_monthly_digest_rollup(tmp_path):
    ...
    assert [d["distro_label"] for d in unsupported] == ["SLES 15"]  # CentOS excluded
...
def test_include_excluded_restores_filtered_distros(tmp_path):
    ...
    assert [d["distro_label"] for d in buckets["known_unsupported"]] == ["CentOS 7", "SLES 15"]
