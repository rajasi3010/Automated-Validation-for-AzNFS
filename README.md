...
| `verdict_source` | Which phase decided: `gate` (Phase 2 repo/package lookup) or `lisa` (Phase 3 ran the suite on a VM). Phase 2 re-checks its own `gate` verdicts every run, so a `known_unsupported` that was stale heals itself; `lisa` verdicts are left alone so a failing distro is not re-provisioned daily. |
